...
UserInfo model)
    {
        var pacienteCad = _context.Colaboradors.FirstOrDefault(e => e.Email == model.Email && e.Cpf == model.Cpf);
        if (pacienteCad != null)
        {
            var user = new ApliccationUser
            {
...
internal class ApliccationUser : ApplicationUser
{
